...
self.addEventListener('fetch', (event) => {
  ...
  // Озвучка: не в PRECACHE — качается при первом ▶ и остаётся в кэше.
  // Range-запросы браузера: отдаём полный файл из кэша (короткие клипы).
  if (/\/audio\//i.test(url.pathname) || /\.m4a$/i.test(url.pathname)) {
